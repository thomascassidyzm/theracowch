...
// `client` exists so a test can drive this to EXHAUSTION without a live Redis.
// The previous pass proved the function fails closed when Redis is absent, which
// is the easy half; nothing proved it actually REFUSES past the ceiling, and a
// spend ceiling nobody has seen refuse is a ceiling nobody should trust. In
// production the parameter is never passed and the module-level client is used.
export async function checkGlobalDailyLimit(bucket, dailyLimit, client = redis) {
