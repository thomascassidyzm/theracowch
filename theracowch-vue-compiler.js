/**
 * Theracowch APML to Vue3 Compiler
 * Deterministic compiler for theracowch anxiety reduction app
 */

const fs = require('fs');
const path = require('path');

class TheracowchVueCompiler {
  constructor() {
    this.appData = {};
  }

  compile(apmlFilePath) {
    const apmlContent = fs.readFileSync(apmlFilePath, 'utf8');
    this.parseAPML(apmlContent);
    return this.generateVueApp();
  }
  ...
  parseAPML(content) {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith('app ')) {
        this.appData.title = trimmed.split(':')[0].replace('app ', '');
      } else if (trimmed.includes('title:')) {
        this.appData.title = trimmed.split('title:')[1].trim().replace(/"/g, '');
      } else if (trimmed.includes('description:')) {
        this.appData.description = trimmed.split('description:')[1].trim().replace(/"/g, '');
      } else if (trimmed.startsWith('interface ')) {
        const interfaceName = trimmed.replace('interface ', '').replace(':', '');
        this.appData.interfaces[interfaceName] = { components: [], elements: [] };
        this.appData.currentInterface = interfaceName;
      } else if (trimmed.startsWith('data ')) {
        const dataName = trimmed.replace('data ', '').replace(':', '');
        this.appData.dataModels[dataName] = { fields: [] };
        this.appData.currentData = dataName;
      } else if (trimmed.includes(':') && this.appData.currentInterface) {
        this.parseInterfaceElement(trimmed);
      } else if (trimmed.includes(':') && this.appData.currentData) {
        this.parseDataField(trimmed);
      }
    }
  }

  parseInterfaceElement(line) {
    const [key, value] = line.split(':').map(s => s.trim());
    if (this.appData.currentInterface) {
      this.appData.interfaces[this.appData.currentInterface].elements.push({
        type: key,
        value: value,
        raw: line
      });
  ...
  parseDataField(line) {
    const [key, value] = line.split(':').map(s => s.trim());
    if (this.appData.currentData) {
      this.appData.dataModels[this.appData.currentData].fields.push({
        name: key,
        type: value,
        raw: line
      });
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { TheracowchVueCompiler };
}

// CLI usage
if (require.main === module) {
  const compiler = new TheracowchVueCompiler();
  const apmlFile = process.argv[2] || 'theracowch-app-specification.apml';
  
  try {
    const vueApp = compiler.compile(apmlFile);
    const outputFile = 'theracowch-compiled-app.html';
    
    fs.writeFileSync(outputFile, vueApp);
    console.log(`🎯 Theracowch Vue App compiled successfully!`);
